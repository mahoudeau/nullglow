...
Reads palette.json and writes every target under dist/. Standard library only —
no toolchain, no node_modules, no build step for people installing the theme.

    python3 build.py            regenerate dist/
    python3 build.py --check    re-verify the accessibility claims and exit

The point of generating rather than hand-maintaining: a role ("pink means
git-dirty and deletions") is defined once, and stays consistent from the editor
to the shell prompt to the diff viewer. Hand-maintained theme repos drift.
"""
...
def emit_fzf(p):
    sh = """# Nullglow for fzf. Source this from your shell rc.
export FZF_DEFAULT_OPTS="
  --height 60%% --layout=reverse --border=rounded --info=inline-right
  --color=bg+:%s,bg:-1,spinner:%s,hl:%s
  --color=fg:%s,header:%s,info:%s,pointer:%s
  --color=marker:%s,fg+:%s,prompt:%s,hl+:%s
  --color=border:%s"
""" % (p.surface, p.green, p.pink, p.fg, p.pink, p.purple, p.green,
       p.green, p.fg, p.green, p.pink, p.line)
    write("fzf/%s.sh" % p.slug, sh)


def emit_zsh(p):
    zsh = """# Nullglow for zsh-syntax-highlighting and zsh-autosuggestions.
# Source AFTER the plugins themselves.

ZSH_AUTOSUGGEST_HIGHLIGHT_STYLE='fg=%s'
...
ZSH_HIGHLIGHT_STYLES[precommand]='fg=%s,italic'
...
""" % (p.comment, p.green, p.green, p.green, p.green, p.green, p.red,
       p.blue, p.yellow, p.yellow, p.comment, p.purple, p.pink)
    write("zsh/%s-highlight.zsh" % p.slug, zsh)
...
def main():
    ...
    for p in palettes:
        ...
        for emit in (emit_vscode, emit_terminal_app, emit_iterm2, emit_bat,
                     emit_starship, emit_vivid, emit_fzf, emit_zsh, emit_delta,
                     emit_preview):
            emit(p)
        print("")
